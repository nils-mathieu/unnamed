...
use starknet_prove_core::{MemoryEntry, MemorySegment, ProofRequest};
use starknet_types_core::felt::Felt;

use crate::{Error, MEMORY_FILE, PRIVATE_INPUT_FILE, PUBLIC_INPUT_FILE, TRACE_FILE};
...
fn serialize_memory_segments<S>(
    memory_segments: &[MemorySegment],
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: Serializer,
{
    serializer.collect_map(memory_segments.iter().map(|seg| {
        (
            seg.name,
            MemorySegmentAddress {
                begin_addr: seg.start,
                stop_ptr: seg.end,
            },
        )
    }))
...
pub fn serialize_private_input(_req: &ProofRequest, buf: &mut Vec<u8>) -> serde_json::Result<()> {
    let val = PrivateInput {
        trace_path: Path::new(TRACE_FILE),
        memory_path: Path::new(MEMORY_FILE),
    };
...
pub async fn write_inputs_to_directory(req: &ProofRequest<'_>, dir: &Path) -> Result<(), Error> {
    let mut buf = Vec::new();
